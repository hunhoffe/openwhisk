...
/**
 * Represents a single log line as read from a ignite log
 * hunhoffe: TODO - customize for ignite log format
 */
protected[core] case class IgniteLogLine(time: String, stream: String, log: String) {
  def toFormattedString = f"$time%-30s $stream: ${log.stripLineEnd}"
}

protected[core] object IgniteLogLine extends DefaultJsonProtocol {
  implicit val serdes = jsonFormat3(IgniteLogLine.apply)
}

object IgniteToActivationLogStore {

  /** Transforms chunked JsObjects into formatted strings */
  val toFormattedString: Flow[ByteString, String, NotUsed] =
    Flow[ByteString].map(_.utf8String.parseJson.convertTo[IgniteLogLine].toFormattedString)
}

/**
 * Ignite based implementation of a LogStore.
 *
 * hunhoffe: TODO
 * Relies on ignites's implementation details with regards to the JSON log-driver. When using the JSON log-driver
 ...
 */
class IgniteToActivationLogStore(system: ActorSystem) extends LogStore {
  ...
  protected def logStream(transid: TransactionId,
    ...
    val logsWithPossibleError = if (isDeveloperError) {
      logs.concat(
        Source.single(
          ByteString(IgniteLogLine(Instant.now.toString, "stderr", Messages.logWarningDeveloperError).toJson.compactPrint)))
    } else logs
  ...
  override def collectLogs(transid: TransactionId,
    ...
    logs
      .via(IgniteToActivationLogStore.toFormattedString)
      .runWith(Sink.seq)
      .flatMap { seq =>
...
object IgniteToActivationLogStoreProvider extends LogStoreProvider {
  override def instance(actorSystem: ActorSystem): LogStore = new IgniteToActivationLogStore(actorSystem)
}
